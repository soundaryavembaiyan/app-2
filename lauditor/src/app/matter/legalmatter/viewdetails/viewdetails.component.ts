...
export class ViewDetailsComponent implements OnInit {
  ...
  updateCorporateNotesVisibility() {
    this.httpservice.sendGetRequest(URLUtils.getLegalHistoryMembers(this.data.id)).subscribe(
      (res: any) => {
        ...
        // this.clientNamesString = this.filteredClients.map((client: any) => client.name).join(', ');
        // Get the first client's name
        // if (this.filteredClients.length > 0) {
        //   this.clientNamesString = this.filteredClients[0].name;
        // } else {
        //   this.clientNamesString = 'No clients available';
        // }
  ...
  // onSubmit() {
  //   //console.log('this.documentDetail',this.documentDetail)
  ...
  onSubmit() {
    //console.log('documentDetail',this.documentDetail.value)
    if (this.documentDetail.value.name == "") {
      this.toast.error('Please enter the document name');
      return;
    }
