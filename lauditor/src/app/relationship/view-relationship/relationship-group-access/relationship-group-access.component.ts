...
export class RelationshipGroupAccessComponent implements OnInit {
  ...
  loadGroups() {
    //For Corporate, Content, Connect products.
    if (this.product !== 'corporate' && this.product !== 'content' && this.product !== 'connect') {
      this.httpService.sendGetRequest(URLUtils.getGroups).subscribe((res: any) => {
        // this.groupList = res?.data;
  ...
  save() {
    ...
    if (this.product === 'corporate') {
      this.httpService.sendPatchRequest(URLUtils.relationshipGroupAclsCorp(this.reldata), payload).subscribe((res: any) => {
        this.event.emit("group-access-done")
      }, handleError)
    }
    //For Content & Connect products.
    else if (this.product === 'content' || this.product === 'connect') {
      this.httpService.sendPutRequest(URLUtils.relationshipGroupAcls(this.reldata), payload).subscribe((res: any) => {
        this.event.emit("group-access-done")
      }, handleError)
    }
    //For Lauditor product.
    else if (this.product !== 'corporate' && this.product !== 'content' && this.product !== 'connect') {
      if (this.activeTab === 'corporate') {
        this.httpService.sendPatchRequest(URLUtils.relationshipGroupAclsCorp(this.reldata), data).subscribe((res: any) => {
          this.event.emit("group-access-done")
        }, handleError)
      }
